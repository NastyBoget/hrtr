...
from data import CTCTokenizer, TransformerTokenizer, EvalSampler, get_data_loader, TrainSampler
...
def val_loop(data_loader, model, tokenizers, device):
    ...
    for tokenizer_name, final_preds in final_predictions.items():
        ...
        wers.append(wer(df["pred"], df["true"]))
        accs.append(string_accuracy(df["pred"], df["true"]))
...
def predict(images, image_masks, model, tokenizers, device):
    """Get attention_model's predicts."""
    model.eval()
...
def run(config):
    ...
    # optimizer = AdamW(attention_model.parameters(), lr=1e-5 / 100, weight_decay=0)
    ...
    if config.paths.path_to_checkpoint is not None:
        print("Loading attention_model from checkpoint")
        ...
        print("Loading attention_model from pretrain")
    ...
    for epoch in range(start_epoch, config.training.num_epochs):
        ...
        cer_avg, _, _ = val_loop(val_loader, model, tokenizers, config.training.device)
        scheduler.step()
        ...
        if cer_avg < best_cer:
            ...
            print(f"Early stopping {early_stopping}/{config.training.early_stopping}")
            early_stopping += 1
...
def run_eval(config, checkpoint_path):
    ...
    cer_avg, wer_avg, acc_avg = val_loop(val_loader, model, tokenizers, config.training.device)
    print('Avg CER:', cer_avg)
    print('Avg WER:', wer_avg)
