...
def _load_original_data(json_path, path) -> List[DataItem]:
    ...
    for img_name, text in orig_data.items():
        itm = RegularDataItem(os.path.join(path, img_name), text, should_remove_alpha=False, should_swap_background=False, can_apply_shadow=True, can_apply_optical_distortion=True)
        items.append(itm)
    return items
