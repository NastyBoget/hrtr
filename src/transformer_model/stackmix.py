...
class StackMix:
    ...
    def run_stackmix(self, text, tokenizer=None):
        ...
        for span in spans:
            ...
            if image is None:
                ...
                path = random.choice(self.token2path.loc[word_token]['path'])
                stack_image = self._load_image(path)
    ...
    def load(self):
        self.stackmix_data = pd.read_csv(f'{self.mwe_tokens_dir}/stackmix_filtered.csv')
        self.stackmix_data['text'] = self.stackmix_data['text'].astype(str)
        self.stackmix_data['path'] = self.stackmix_data['path'].apply(lambda x: self.mwe_tokens_dir + '/' + '/'.join(x.split('/')[-3:]))
        for max_len in range(3, self.max_token_size+1):
            mwes = sorted([mwe for mwe in self.stackmix_data['text'].unique() if len(mwe) <= max_len])
            self.tokenizers[max_len] = MWETokenizer(mwes=mwes, separator='')
    ...
    def load_corpus(self):
        ...
        lines = open(f'{self.mwe_tokens_dir}/lexicon.txt', 'r').readlines()
