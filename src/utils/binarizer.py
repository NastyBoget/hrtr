...
class Binarizer:
    ...
    def binarize(self, img: np.ndarray) -> np.ndarray:
        ...
        image_out = self._combine_block_image_process(image_in, mask)
    ...
    def _get_block_index(self, image_shape, yx):
    ...
    def _block_image_process(self, image: np.ndarray) -> np.ndarray:
        out_image = np.zeros_like(image)
        for row in range(0, image.shape[0], self.block_size):
            for col in range(0, image.shape[1], self.block_size):
                idx = (row, col)
                block_idx = self._get_block_index(image.shape, idx)
    ...
    def _get_mask(self, img: np.ndarray) -> np.ndarray:
        ...
        image_out = self._block_image_process(image_in)
    ...
    def _combine_block(self, img_in: np.ndarray, mask: np.ndarray):
    ...
    def _combine_block_image_process(self, image, mask):
        out_image = np.zeros_like(image)

        for row in range(0, image.shape[0], self.block_size):
            for col in range(0, image.shape[1], self.block_size):
                block_idx = self._get_block_index(image.shape, (row, col))
