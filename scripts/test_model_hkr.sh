...
CUDA_VISIBLE_DEVICES=1 python3 src/test.py --test_data $DATA_DIR/test1/$DATASET --select_data $DATASET \
...
CUDA_VISIBLE_DEVICES=1 python3 src/test.py --test_data $DATA_DIR/test2/$DATASET --select_data $DATASET \
...
CUDA_VISIBLE_DEVICES=1 python3 src/test.py --test_data $DATA_DIR/test1/$DATASET --select_data $DATASET \
  --log_dir $LOGS_DIR --log_name test1_$DATASET\_augmentation.txt --write_errors --saved_model $ROOT/saved_models/$DATASET\_augmentation/best_loss.pth \
  --transformation TPS --feature_extraction ResNet --sequence_modeling BiLSTM --prediction Attn --sensitive

CUDA_VISIBLE_DEVICES=1 python3 src/test.py --test_data $DATA_DIR/test2/$DATASET --select_data $DATASET \
  --log_dir $LOGS_DIR --log_name test2_$DATASET\_augmentation.txt --write_errors --saved_model $ROOT/saved_models/$DATASET\_augmentation/best_loss.pth \
  --transformation TPS --feature_extraction ResNet --sequence_modeling BiLSTM --prediction Attn --sensitive
